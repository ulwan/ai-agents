...
- Use the `flight_seat_selection_agent` tool to flight find seat choices,
- Use the `hotel_search_agent` tool to find hotel choices,
- Use the `hotel_room_selection_agent` tool to find hotel room choices,
...
Current time: {system_time}; Infer the current Year from the time.
...
FLIGHT_SEARCH = """Generate search results for flights from origin to destination inferred from user query please use future dates within 3 months from today's date for the prices, limit to 4 results.
  ...
  {{"flights": [
    {
      ...
      "airlines": [
        "Airline names, e.g., Garuda Indonesia, Emirates"
      ],
      "price_in_rp": "Integer - Flight price in Indonesian Rupiah",
...
HOTEL_SEARCH = """Generate search results for hotels for hotel_location inferred from user query. Find only 4 results.
...
- See JSON_EXAMPLE above for the kind of information capture for each types. 
  ...
  - For hotels, include:
    ...
    - Note the hotel price should be the total amount covering all nights in indonesian rupiah (IDR).
    - e.g. {{
        "event_type": "hotel",
        "description": "Seattle Marriott Waterfront",
        "address": "2100 Alaskan Wy, Seattle, WA 98121, United States",
